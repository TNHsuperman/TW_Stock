...
st.set_page_config(page_title="台股快速選股器", layout="centered")
            ...
            try:
                ...
                if keep:
                    ...
                    results.append({
                        ...
                        "類股": stock_data["industry"], "收盤": round(close, 2), "指標": score_val
                    })
            except: continue
        progress_bar.progress(min((i + batch_size) / len(all_stocks), 1.0))
    
    st.session_state['scan_results'] = pd.DataFrame(results)
    st.session_state['current_idx'] = 0  # 初始化索引
...
if 'scan_results' in st.session_state and not st.session_state['scan_results'].empty:
    ...
    # 防止切換類股後索引溢出
    if 'current_idx' not in st.session_state or st.session_state.current_idx >= len(df_filtered):
    ...
    if col1.button("⬅️ 上一支", use_container_width=True):
        st.session_state.current_idx = (st.session_state.current_idx - 1) % len(df_filtered)
        
    if col3.button("下一支 ➡️", use_container_width=True):
        st.session_state.current_idx = (st.session_state.current_idx + 1) % len(df_filtered)

    # 取得當前選中的股票資訊
    current_row = df_filtered.iloc[st.session_state.current_idx]
    ticker_id = current_row['ID']
    stock_name = current_row['名稱']
    
    col2.markdown(f"<h3 style='text-align: center;'>{stock_name} ({ticker_id})</h3>", unsafe_allow_html=True)
    col2.write(f"<p style='text-align: center;'>進度：{st.session_state.current_idx + 1} / {len(df_filtered)}</p>", unsafe_allow_html=True)

    # 繪製 K 線圖
    with st.spinner(f'正在載入 {stock_name} ...'):
        ...
        fig.update_layout(
            xaxis_rangeslider_visible=False, height=500, margin=dict(l=5, r=5, t=10, b=10),
...
# --- 側邊欄腳註 ---
st.sidebar.markdown("---")
st.sidebar.write(f"最後更新時間：2026-01-24")
