...
if not st.session_state['scan_results'].empty:
    ...
    if len(df_filtered) > 0:
        ...
        with st.spinner(f'載入中... {row["名稱"]} ({row["ID"]})'):
            ...
            # 確保資料沒有空值並重設 index 將日期變為字串以消除間隔
            df_p = df_p.dropna(subset=['Close'])
            # 重要：將 index 轉換為字串格式，Plotly 就不會去補週末空白
            ...
            # 精確買點標記
            fig.add_trace(go.Scatter(x=df_p['Date_Str'], y=df_p['Buy_Marker'], mode='markers',
            ...
            # 佈局美化與 X 軸類別設定 (關鍵：type='category')
            fig.update_xaxes(type='category', showgrid=True, gridcolor='rgba(200,200,200,0.2)', tickangle=-45, nbinsx=20)
            fig.update_layout(
